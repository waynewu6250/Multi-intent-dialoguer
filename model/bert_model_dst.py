...
class BertDST(nn.Module):
    ...
    def __init__(self, config, opt, num_labels=2):
        ...
        self.rnn = nn.LSTM(input_size=768, 
                           ...
                           batch_first=True, 
                           num_layers=1)
        self.top_layer = nn.Linear(512, self.num_labels)
        self.opt = opt
    ...
    def forward(self, x_caps, x_masks, y_caps, y_masks, labels, X_lengths):
        """
        BERT outputs:
        ...
        # Bert recursive model
        dialog_size = self.opt.max_dialog_size
        b, t = x_caps.shape
        num_samples = int(b / dialog_size)
        
        # x_caps_new = x_caps.view(num_samples, dialog_size, -1)
        # x_masks_new = x_masks.view(num_samples, dialog_size, -1)

        _, pooled_output, _, _ = self.bert(x_caps, attention_mask=x_masks)

        # hiddens = torch.zeros(num_samples, dialog_size, 768).to(self.device)
        # for i in range(dialog_size):
        #     xi = x_caps_new[:, i, :] # (b, t)
        #     mi = x_masks_new[:, i, :] # (b, t)
        #     _, pooled_output, _, _ = self.bert(xi, attention_mask=mi) # (b, h)
        #     hiddens[:, i, :] = pooled_output
        hiddens = pooled_output.view(num_samples, dialog_size, -1)
        
        # 1
        # hiddens = hiddens.transpose(0,1)
        # rnn_out, _ = self.rnn(hiddens)
        # rnn_out = rnn_out.transpose(0,1)
        # logits = self.top_layer(rnn_out)
        ...
        # rnn_out, _ = self.rnn(hiddens)
        # rnn_out = rnn_out.transpose(0,1)
        # rnn_out = rnn_out.reshape(-1, 768)
        # logits = self.multi_learn(rnn_out, clusters, labels)

        # 3
        # We only take 1 out of 25 of X_lengths
        idx = torch.arange(0, len(X_lengths), self.opt.max_dialog_size)
        X_lengths = X_lengths[idx]

        X = torch.nn.utils.rnn.pack_padded_sequence(hiddens, X_lengths, batch_first=True, enforce_sorted=False)
        output_packed, _ = self.rnn(X)
        rnn_out, output_lengths = torch.nn.utils.rnn.pad_packed_sequence(output_packed, batch_first=True)
        logits = self.top_layer(rnn_out)
        logits = logits.view(-1, self.num_labels)

        return hiddens, pooled_output, logits
    ...
    def multi_learn(self, pooled_output, clusters, labels):
        ...
        if self.mode2 == 'gram':
            ...
            query = pooled_output.unsqueeze(1).repeat(1, self.num_labels, 1)
            support = clusters.unsqueeze(0).repeat(b, 1, 1)
